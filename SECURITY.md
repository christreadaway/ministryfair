...
Look in `index.html` around line 515. It should only say:
```javascript
...
```bash
# If the key is in index.html, replace it with the placeholder
# Edit index.html and change the line back to:
apiUrl: 'YOUR_GOOGLE_APPS_SCRIPT_URL_HERE',
```
...
3. **Use environment variables**: For deployed apps, use environment variables for secrets
4. **Regenerate exposed keys**: If a key is ever exposed, regenerate it immediately
5. **Use Google Apps Script**: For this app, you only need the Apps Script URL (which is safe to be public)
...
If you're not comfortable with these technical steps:

1. Ask a tech-savvy friend or parishioner to help
2. Contact your parish IT coordinator
3. Reach out to the developer community for this project

Remember: **When in doubt, regenerate the key.** It's better to break something temporarily than to leave a security risk active.
